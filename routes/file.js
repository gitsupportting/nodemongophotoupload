...
/** ************************ file uplaod and download route *************************/
/* file upload : POST */
router.post('/files/otherarticleimg/:filename', upload.single('file'), function (req, res) {
  if (req.file) {
    var article_id = req.body.article_id;
    var filename = req.params.filename;
    ...
    collection.find().toArray(function (_err, docs) {
      query = { 'image_id': uniqid.time(), 'article_id': article_id, 'filename': filename };
      collection.insertOne(query, function (_err, inserted) {
      });
    });
    res.json(_displayResults(_resultCode.FILE_UPLOAD_SUCCESS, 'Successfully uploaded', true));
...
router.post('/getdata/addarticle/:filename', upload.single('file'), function (req, res) {
  if (req.file) {
    ...
    collection.find(query).toArray(function (_err, docs) {
      if (docs.length === 0) {
        query = { 'article_id': articleId, 'title': title, 'content': content, 'status_b': status_b, 'dateTime': dateTime, 'mainImage': mainImage, 'othersImg': '', 'sticky': sticky };
        collection.insertOne(query, function (_err, inserted) {
...
/** ************************ file uplaod and download route *************************/
/* file upload : POST */
router.post('/getdata/editarticle/:filename', upload.single('file'), function (req, res) {
  var article_id = req.body.article_id;
  if (article_id === undefined) {
    res.status(405).json(_displayResults(_resultCode.TITLE_UNDEFINED, 'article_id is undefined'));
    return;
  }
  var title = req.body.title;
  if (title === undefined) {
  ...
  setTimeout(() => {
    collection.find().toArray(function (_err, docs) {
      collection.findOneAndUpdate(
        ...
        { '$set': { 'title': title, 'content': content, 'status_b': status_b, 'dateTime': dateTime, 'mainImage': mainImage, 'othersImg': '', 'sticky': sticky } },
        function (err, doc) {
          if (err) { // err: any errors that occurred
            res.send(err);
          } else {
